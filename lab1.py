...
def AuksinisPjuvis(fun, prad, pab, min):
    ...
    def vidus(l, X1, X2, r, tikslof = 0, iter = 1, fX1 = None, fX2 = None): #################
        if(fX1 is None):
            fX1 = fun(X1)
            tikslof += 1 #################
        if(fX2 is None):
        ...
        if(L < min):
            ...
            return vidus(l, X1, X2, r, tikslof, iter + 1, fX1, fX2) #############
...
def NiutonoMetodas(funD, funDD, min, X0):  #######################
    def vidus(X0, iter = 0, tikslof = 0):
        ...
        # taskas += 1
        iter += 1 ####################################
        print(f"{iter} {tikslof} {(X0,Xn)} {X0 - Xn}")    ##############################
        if(abs(X0 - Xn) < min):
            return Xn
        else:
            return vidus(Xn, iter, tikslof)
...
NiutonoMetodas(funkcijaD, funkcijaDD, e, 5)